...
class MyTheme_Contact_Info_Widget extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'Hiển thị thông tin liên hệ', 'medis' ),
		);
	...
	public function form( $instance ): void {
		$defaults = [
			'title'   => esc_html__( 'Thông tin liên hệ', 'medis' ),
			'address' => '',
			'email'   => '',
			'phone'   => ''
		];
		...
		$fields = [
			'title'   => esc_html__( 'Tiêu đề:', 'medis' ),
			'address' => esc_html__( 'Địa chỉ:', 'medis' ),
			'email'   => esc_html__( 'Email:', 'medis' ),
			'phone'   => esc_html__( 'Số điện thoại:', 'medis' )
		];
...
function medis_register_contact_info_widget(): void {
	register_widget( 'MyTheme_Contact_Info_Widget' );
}

add_action( 'widgets_init', 'medis_register_contact_info_widget' );